...
class PINN(nn.Module):
    def __init__(self,
        ...
        self.Bty = torch.randn((1, n_mode_spacey), device=device) * torch.sigmoid(torch.arange(n_mode_spacey, device=device)**2)
...
def train_model(
    ...
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, nn_approximator.parameters()), nn_approximator.parameters(), lr = learning_rate)
