...
def initial_conditions(space: torch.Tensor, w0: float) -> torch.tensor:
    ...
    uy0 = w0 * (torch.sin(2*torch.pi*x) + 2*torch.sin(8*torch.pi*x))
    dotux0 = torch.zeros_like(x)
    dotuy0 = -10*w0 * (torch.sin(2*torch.pi*x) + torch.sin(5*torch.pi*x))
...
class PINN(nn.Module):
    def __init__(self,
        ...
        for i in range(len(modesy)):
            By = modesy[i] * torch.randn([2, n_mode_spacey], device=device) + torch.rand(1, device=device).item()
...
class Loss:
    ...
    def res_loss(self, pinn, use_init: bool = False):
        ...
        return 3e-4 * loss, V, T, errV, errT, loss_kurt, loss_skew, lossesall.detach()
...
def train_model(
    ...
    for epoch in range(max_epochs + 1):
        ...
        l1_penalty = sum(param.abs().sum() for param in nn_approximator.parameters())
        lam = 1e-6 
        loss += lam * l1_penalty
...
def obtainsolt_u(pinn: PINN, space: torch.Tensor, t: torch.Tensor, nsamples: tuple, hypert: int, par: dict, steps: list, device: torch.device):
    ...
    return sol.detach().cpu().numpy(), V.detach().cpu().numpy(), T.detach().cpu().numpy(), v.reshape(nx, ny, nt, 2).detach().cpu().numpy()
